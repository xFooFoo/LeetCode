...
class Solution:
    def reverseOddLevels(self, root: Optional[TreeNode]) -> Optional[TreeNode]:
        def traverse(nodeL: Optional[TreeNode], nodeR: Optional[TreeNode], oddLevel: bool):
            if not nodeL or not nodeR:
                return
            # Reverse values if the level is odd
            if oddLevel:
                nodeL.val, nodeR.val = nodeR.val, nodeL.val
            traverse(nodeL.left, nodeR.right, not oddLevel)
            traverse(nodeL.right, nodeR.left, not oddLevel)
